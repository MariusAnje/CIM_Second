...
if __name__ == "__main__":
    ...
    if not args.pretrained:
        train(args.train_epoch, header, args.verbose)
    ...
    if args.use_mask:
        ...
        print(f"with mask no noise: {eval():.4f}")
        # GetSecond()
        ...
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer, [20])
        train(args.fine_epoch, header_timer, args.verbose)

        if args.save_file:
            torch.save(model.state_dict(), f"saved_A_{header}_{header_timer}.pt")
        fine_mask_acc_list = []
        print(f"Finetune no noise: {eval():.4f}")
        loader = range(args.noise_epoch)
        for _ in loader:
            acc = eval_noise(args.noise_var)
